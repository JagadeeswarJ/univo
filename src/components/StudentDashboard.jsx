...
import { UserCircle, Award, BadgeCheck, FileText, Calendar, BookOpen, Code, Server, Database, Layers, ChevronDown, ChevronUp, X } from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  const maxActivity = Math.max(...weeklyActivityData.map(item => item.problems));
  const totalProblems = weeklyActivityData.reduce((sum, day) => sum + day.problems, 0);
...
const { monthlyPerformance, weeklyPerformance } = generatePerformanceData();
  ...
  const eventTypes = [
    { name: "Hackathons", count: 5, color: "bg-emerald-500", colorHex: "#10b981" },
    { name: "Workshops", count: 8, color: "bg-amber-500", colorHex: "#f59e0b" },
    { name: "Competitions", count: 12, color: "bg-sky-500", colorHex: "#0ea5e9" },
    { name: "Seminars", count: 4, color: "bg-fuchsia-500", colorHex: "#d946ef" },
  ];
  ...
  const allEvents = [
    { 
      ...
      date: "12 March 2024", 
      status: "Completed",
      description: "A 48-hour hackathon focused on creating innovative solutions for climate change using emerging technologies. Teams of 3-5 participants worked on projects involving AI, IoT, and blockchain.",
    ...
    { 
      ...
      date: "12 April 2024", 
      status: "Registered",
      description: "An expo showcasing the latest advancements in artificial intelligence and machine learning. Features keynote speakers from leading tech companies and research institutions.",
    ...
    { 
      ...
      date: "18 March 2024", 
      status: "View Details",
      description: "A competitive coding challenge where participants solve algorithmic problems within a time limit. Problems range from easy to hard difficulty levels, testing various aspects of computer science knowledge.",
    ...
    { 
      ...
      date: "24 April 2024", 
      status: "Registered",
      description: "Hands-on workshop covering data analysis, visualization, and machine learning techniques using Python. Participants will work with real-world datasets to build predictive models.",
    ...
    { 
      ...
      date: "5 May 2024", 
      status: "Registered",
      description: "A summit focused on cloud architecture, serverless computing, and DevOps practices. Includes technical sessions and hands-on labs with major cloud providers.",
    ...
    { 
      ...
      date: "2 March 2024", 
      status: "Completed",
      description: "Intensive 2-day bootcamp covering modern web development frameworks and best practices. Topics included React, Node.js, and responsive design principles.",
  ...
  const calculatePieSegments = () => {
    ...
    return eventTypes.map(type => {
      ...
      const segmentPath = describeArc(50, 50, 40, startAngle, startAngle + degrees);
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 bg-gray-50 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <div className="flex border-b border-gray-200">
          <button 
            ...
            className={`px-4 py-2 font-medium text-sm ${activeTab === 'overview' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-600'}`}
          >
            Overview
          </button>
          <button 
            onClick={() => setActiveTab('performance')} 
            className={`px-4 py-2 font-medium text-sm ${activeTab === 'performance' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-600'}`}
          >
            Performance
          </button>
          <button 
            onClick={() => setActiveTab('events')} 
            className={`px-4 py-2 font-medium text-sm ${activeTab === 'events' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-600'}`}
          >
            Events
          </button>
          <button 
            onClick={() => setActiveTab('certificates')} 
            className={`px-4 py-2 font-medium text-sm ${activeTab === 'certificates' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-600'}`}
          >
            Certificates
          </button>
        </div>
        ...
        <>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-1 flex flex-col items-center lg:items-start">
              ...
              <h3 className="text-2xl font-semibold text-gray-800">Vi Sen</h3>
            ...
            <div className="lg:col-span-3 grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Stat icon={<Award className="w-5 h-5" />} label="Events" value="3" />
              <Stat icon={<FileText className="w-5 h-5" />} label="Participations" value="12" />
              <Stat icon={<BadgeCheck className="w-5 h-5" />} label="Certificates" value="4" />
              <Stat icon={<BookOpen className="w-5 h-5" />} label="Problems Solved" value="247" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-semibold text-gray-800">Weekly Activity</h4>
                <div className="text-sm">
                  <span className="font-medium text-indigo-600">{totalProblems}</span> problems solved · 
                  <span className="font-medium text-indigo-600 ml-1">{totalHours}</span> hours
                </div>
              </div>
              <div className="h-64">
                  ...
                  {weeklyActivityData.map((item, index) => (
                    <div key={index} className="flex flex-col items-center flex-1">
                      <div 
                        ...
                        style={{ height: `${(item.problems / maxActivity) * 100 || 10}%`, minHeight: '10px' }}
                      >
      ...
      {activeTab === 'performance' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 lg:col-span-1">
            <h4 className="text-lg font-semibold text-gray-800 mb-6">Event Participation</h4>
            <div className="flex justify-center mb-8">
              <div className="relative w-64 h-64">
                <svg viewBox="0 0 100 100">
                  ...
                  <text x="50" y="46" textAnchor="middle" fontSize="8" fontWeight="bold" fill="#333333">
                    {totalEvents}
                  </text>
                  <text x="50" y="56" textAnchor="middle" fontSize="5" fill="#666666">
                    Total Events
                  </text>
                </svg>
        ...
        {/* Tech Stack */}
                  <div className="bg-white rounded-2xl shadow-lg p-6 lg:col-span-2">
                    <h4 className="text-lg font-semibold text-gray-800 mb-4">Tech Stack Proficiency</h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                      <TechStackCard icon={<Code />} name="Frontend" percentage={92} />
                      <TechStackCard icon={<Server />} name="Backend" percentage={78} />
                      <TechStackCard icon={<Database />} name="Database" percentage={65} />
                    ...
                    <div className="h-64 flex items-end">
                        <div className="flex-1 flex items-end space-x-2">
                            {weeklyPerformance.map((data, i) => {
                                const maxProblems = Math.max(...weeklyPerformance.map(item => item.problems));
                                const barHeight = (data.problems / maxProblems) * 100 || 10; // Ensure minimum height for visibility
                                return (
                                    <div key={i} className="flex flex-col items-center flex-1">
                                        <div 
                                            className={`w-full ${i % 2 === 0 ? 'bg-indigo-600' : 'bg-indigo-400'} rounded-t-md hover:bg-indigo-500 transition-all cursor-pointer relative group`}
                    ...
                    <div className="h-64 flex items-end">
                      <div className="flex-1 flex items-end space-x-2">
                        {monthlyPerformance.map((data, i) => {
                          const maxEvents = Math.max(...monthlyPerformance.map(item => item.events));
              ...
              <div className="flex-1 flex items-end space-x-2">
                {monthlyPerformance.map((data, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center flex-1">
                      <div 
                        className={`w-full ${i === monthlyPerformance.length - 1 ? 'bg-indigo-600' : 'bg-indigo-400'} rounded-t-md hover:bg-indigo-500 transition-all cursor-pointer relative group`}
                        style={{ height: `${barHeight}%` }}
                      >
                        <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                          {data.events} events
      ...
      {activeTab === 'events' && (
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <h4 className="text-lg font-semibold text-gray-800 mb-6">All Events</h4>
          <div className="space-y-4">
            {allEvents.map((event) => (
              <EventCard 
                key={event.id} 
...
const Stat = ({ icon, label, value }) => (
  <div className="flex flex-col items-center bg-gray-50 rounded-xl p-4 transition-all hover:bg-indigo-50 hover:shadow-md">
    <div className="text-indigo-600 mb-2 bg-indigo-100 p-2 rounded-lg">{icon}</div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {event.certification && (
              <div className="bg-gray-50 p-4 rounded-lg">
                <h4 className="font-medium text-gray-800 mb-1">Certification</h4>
